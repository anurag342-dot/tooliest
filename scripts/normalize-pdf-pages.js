...
const PDF_OVERRIDES = {
  ...
  'pdf-compressor': {
    ...
    howToSteps: [
      ...
      { name: 'Run the compression pass', text: 'Process the PDF and let Tooliest compare multiple browser-safe compression strategies instead of forcing a one-size-fits-all export.' },
      { name: 'Keep the smallest valid result', text: 'Review the original versus result size and download the file only when Tooliest actually beats the original size.' },
    ],
    faq: [
      ...
      { q: 'Will every PDF get smaller after compression?', a: 'No. Text-heavy or vector PDFs are sometimes already more efficient than a rasterized copy, so Tooliest warns you instead of pretending a larger file is a successful compression.' },
  ...
  'pdf-protect': {
    ...
    howToSteps: [
      ...
      { name: 'Encrypt the document in your browser', text: 'Run the protection step to add real password-based access to the PDF locally on your device.' },
    ...
    faq: [
      ...
      { q: 'Will the downloaded PDF actually ask for a password?', a: 'Yes. The exported file includes a PDF encryption dictionary, so compatible readers prompt for the password before the document opens.' },
